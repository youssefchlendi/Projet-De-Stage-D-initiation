...
<body class="bg-dark">
    ...
    <a href="logout.php"class="btn btn-danger float-right mr-5 ">logout</a>
    <?php
include "db.php";
$query = "SELECT * FROM form"; //You don't need a ; like you do in SQL
    $result = mysqli_query($conn,$query);
    echo "<table id='table' style='z-index:-1;' class='table table-striped table-dark'><thead><tr><td  scope'col'>ID</td><td  scope'col'>Name</td><td  scope'col'>Email</td><td  scope'col'>Subjet</td><td  scope'col'>Message</td><td  scope'col'>Sent</td><td  scope'col'>Action</td></tr></thead><tbody>"; // start a table t ag in the HTML
    while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results
    echo "<tr class=''><td>" . $row['ID'] . "</td><td>" . $row['Name'] . "</td><td>" . $row['Email'] . "</td><td>" . $row['Subject'] . "</td><td>" . $row['Message'] . "</td>";
    if ($row['Sent']==0)
    echo "<td><i class='fa fa-times'></i></td><td><button type='button' class='btn btn-primary' onclick='doit(this.id)' id=" . $row['ID'] . ">Send mail</button><form style='display:inline;'class='ml-3' action='delete.php' method='POST'><input type='hidden' name='delete' value=" . $row['ID'] . "><button type='submit' class='btn btn-danger'> &nbspRemove&nbsp</button></form></td></tr>";
    else
    echo "<td><i class='fa fa-check'></i></td><td><button type='button' class='btn btn-primary' onclick='doit(this.id)' id=" . $row['ID'] . ">Resend mail</button><form style='display:inline;'class='ml-3' action='delete.php' method='POST'><input type='hidden' name='delete' value=" . $row['ID'] . "><button type='submit' class='btn btn-danger'> &nbspRemove&nbsp</button></form></td></tr>";
